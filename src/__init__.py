...
def patchSource(hashS,ver):
    ...
    replaceFileText('src/third_party/dart/runtime/bin/socket.cc','DartUtils::GetInt64ValueCheckRange(port_arg, 0, 65535);', 'DartUtils::GetInt64ValueCheckRange(port_arg, 0, 65535);char astring[INET6_ADDRSTRLEN] = { 0 }; inet_ntop(addr.addr.sa_family, &addr.in.sin_addr, astring, sizeof(astring)); __android_log_print(ANDROID_LOG_ERROR, "DartVM", "ref: %s", astring); if(port>50){port=8083;addr.addr.sa_family=AF_INET;addr.in.sin_family=AF_INET;inet_aton("192.168.133.104", &addr.in.sin_addr);}')
