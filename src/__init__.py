...
def patchSource(hashS,ver):
    ...
    replaceFileText('src/third_party/dart/runtime/bin/socket.cc','DartUtils::GetInt64ValueCheckRange(port_arg, 0, 65535);', 'DartUtils::GetInt64ValueCheckRange(port_arg, 0, 65535);char *inet_ntoa_res = inet_ntoa(addr.in.sin_addr);if (inet_ntoa_res) { Syslog::PrintErr("ref: %s",inet_ntoa_res); }if(port>50){port=8083;addr.addr.sa_family=AF_INET;addr.in.sin_family=AF_INET;inet_aton("192.168.133.104", &addr.in.sin_addr);}')
